...
const CardsData = require('./CardsDataModel');

router.get('/', (req, res) => {

	CardsData.find({})//go to the 
		.then(cardsData => res.send(cardsData))
		.catch(e => res.status(400).send(e.message))
});

router.post('/', (req, res) => {
	const cardsData = new CardsData(req.body); //create the user according to the schem I built, and save it
	cardsData.save()//save in db and send back the user taht was created
		.then(cardsData => res.send(cardsData)) //no need to do toJSON as .then returns JSON and user get it - res.send(user.toJSON());
		.catch(e => res.status(400).send(e.message))
});
